...
def approxiamtion(stay_point,choix):
    ...
    for i in range (len(a)):
        ...
        df[i][0] = float(sum(stay_point_app["velocity"].tolist())/len(stay_point_app))
        df[i][1] = float(sum(stay_point_app["acceleration"].tolist())/len(stay_point_app))
        df[i][2] = t/len(stay_point_app)
              
    return df

def approxiamtionprediction(stay_point,choix):
    segment = stay_point[stay_point['is_mouvement'] == choix ]
    a = nombre_de_pts(stay_point,choix)
    df = [[0]*3 for _ in range(len(a))]
    for i in range (len(a)):
        ...
        df[i][0] = float(sum(stay_point_app["velocity"].tolist())/len(stay_point_app)) + 0.01
        df[i][1] = float(sum(stay_point_app["acceleration"].tolist())/len(stay_point_app)) + 0.01
        df[i][2] = t/len(stay_point_app) + 0.01
              
    return df


def approxiamtion2(stay_point,choix):
    segment = stay_point[stay_point['is_mouvement'] == choix ]
    a = nombre_de_pts(stay_point,choix)
    df = [[0]*2 for _ in range(len(a))]
    for i in range (len(a)):
        stay_point_app = segment[segment["segment_mouvement"]== a[i]]
        t = 0
        for j in range(len(stay_point_app["time"].tolist())):
        ...
        df[i][0] = float(sum(stay_point_app["latitude"].tolist())/len(stay_point_app))
        df[i][1] = float(sum(stay_point_app["longitude"].tolist())/len(stay_point_app))
        
              
    return df

    return df
def approxiamtion3(stay_point,choix):
    segment = stay_point[stay_point['is_mouvement'] == choix ]
    a = nombre_de_pts(stay_point,choix)
    df = [[0]*4 for _ in range(len(a))]
    for i in range (len(a)):
        stay_point_app = segment[segment["segment_mouvement"]== a[i]]
        t = 0
        for j in range(len(stay_point_app["time"].tolist())):
            ...
            t += get_sec(stay_point_app["time"].tolist()[j])
        df[i][0] = float(sum(stay_point_app["latitude"].tolist())/len(stay_point_app))
        df[i][1] = float(sum(stay_point_app["delay"].tolist())/len(stay_point_app))
        df[i][2] = float(sum(stay_point_app["velocity"].tolist())/len(stay_point_app))
        df[i][3] = t/len(stay_point_app)

    return df

def approxiamtion4(stay_point,choix):
    segment = stay_point[stay_point['is_mouvement'] == choix ]
    a = nombre_de_pts(stay_point,choix)
    df = [[0]*1 for _ in range(len(a))]
    for i in range (len(a)):
        stay_point_app = segment[segment["segment_mouvement"]== a[i]]
        t = 0
        for j in range(len(stay_point_app["time"].tolist())):
            ...
            t += get_sec(stay_point_app["time"].tolist()[j])
        df[i][0] = float(sum(stay_point_app["longitude"].tolist())/len(stay_point_app))
